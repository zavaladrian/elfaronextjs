        ...
        <div className="mt-2 space-y-12 lg:grid lg:grid-cols-5 lg:gap-x-8 lg:space-y-0">
          {features.map((feature) => (
            <a className="group block" key={feature.id}>
              ...
              <br />
              <h2 className="mt-4 text-xl sm:text-4xl font-semibold font-playfair text-center text-black">
                {feature.name}
              </h2>
              <p className="mt-2 text-lg sm:text-xl text-center font-dosis text-gray-600 text-balance">
                {feature.description}
              </p>
              <p className="mt-2 text-lg sm:text-base text-center font-dosis text-rose-900 text-pretty">
                {feature.available}
              </p>
            </a>
          ))}
